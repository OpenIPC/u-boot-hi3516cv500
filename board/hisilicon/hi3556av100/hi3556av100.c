	...
	if (auto_update_flag) {
		...
		if (result == 0) {
			...
			if ((uregval & UPGRADE_STATUS_PROCESSING) != NUM_0) {
				writel(UPGRADE_STATUS_FINISH,
						UPGRADE_STATUS_REG_ADDR);
...
static int start_a53cpu1(cmd_tbl_t *cmdtp, int flag, int argc,
		char *const argv[])
{
	int ret = 0;
	unsigned int addr;
	...
	ret = hw_dec_decompress(NULL, (unsigned char *)(uintptr_t)addr, &size_uncompressed,
			NULL, ((unsigned char *)(uintptr_t)srcaddr +
				HIGZIP_HEAD_SIZE), size_compressed, NULL);
	if (ret)
