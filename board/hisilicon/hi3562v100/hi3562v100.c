...
int save_bootdata_to_flash(void)
{
    ...
    if (sd_update_flag == START_MAGIC) {
        ...
        if (boot_media == BOOT_MEDIA_SPIFLASH) {
            ...
            if (val) {
                printf("SPI flash write failed, return %x\n",
                       val);
        ...
        if (boot_media == BOOT_MEDIA_NAND) {
            ...
            if (val) {
                printf("Nand flash write failed, return %x\n",
                       val);
                unmap_physmem(buf, UBOOT_DATA_SIZE);
                return 1;
            }
        ...
        if (boot_media == BOOT_MEDIA_EMMC) {
            struct mmc *mmc = find_mmc_device(NUM_0);
            ...
            printf("MMC write...\n");
            blk_dwrite(mmc_get_blk_desc(mmc), NUM_0, (UBOOT_DATA_SIZE >> NUM_9), buf);
            unmap_physmem(buf, UBOOT_DATA_SIZE);
        }
#endif
...
int misc_init_r(void)
{
	...
	if (auto_update_flag) {
        ...
        unsigned int* puregval = (unsigned int*)UPGRADE_STATUS_REG_ADDR;
		if ((((*puregval) & (UPGRADE_STATUS_PROCESSING << NUM_8)) != NUM_0) && (result == NUM_0)) {
...
static int start_other_cpus(cmd_tbl_t *cmdtp, int flag, int argc,
						char *const argv[])
{
    int ret = 0;
	unsigned int addr;
    ...
    ret = hw_dec_decompress((unsigned char *)(uintptr_t)addr, &size_uncompressed, \
                            ((unsigned char *)(uintptr_t)srcaddr + HIGZIP_HEAD_SIZE), size_compressed, NULL);
    if (!ret) {
