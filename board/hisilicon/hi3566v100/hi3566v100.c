...
int data_to_nandflash(void)
{
	...
	unsigned int val;
            ...
            nand_flash = nand_info[NUM_0];

            printf("Nand flash erase...\n");
            val = nand_erase(nand_flash, NUM_0, UBOOT_DATA_SIZE);
            if (val) {
            ...
            val = nand_write(nand_flash, NUM_0, &length, buf);
            if (val) {
...
int misc_init_r(void)
{
	...
	if (result == NUM_0)
        do_reset(NULL, NUM_0, NUM_0, NULL);
#endif
    return 0;
}
...
static int start_other_cpus(cmd_tbl_t *cmdtp,
