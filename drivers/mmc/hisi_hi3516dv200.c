...
static void hisi_mmc_set_ioconfig(struct sdhci_host *host)
{
	...
	if (host->type == MMC_TYPE_MMC) {
		...
		case MMC_TIMING_MMC_HS200:
			hisi_set_drv_str(REG_CTRL_EMMC_CLK, 0, 1, 0, 0x2);  /* set drv level 3 */
		...
		case MMC_TIMING_MMC_HS:
			hisi_set_drv_str(REG_CTRL_EMMC_CLK, 0, 1, 1, 0x4);  /* set drv level 3 */
		...
		default:
			...
			hisi_set_drv_str(REG_CTRL_EMMC_CMD, 1, 0, 1, 0x6);  /* set drv level 5 */
			for (i = 0; i < IO_CFG_EMMC_DATA_LINE_COUNT; i++)
				hisi_set_drv_str(
					io_emmc_data_reg[i], 1, 0, 1, 0x6); /* set drv level 6 */
