...
static int mmc_startup(struct mmc *mmc)
{
	...
	if (mmc->ocr_from_bootrom) {
		/* Active is transfer state, deselect the card, and put it into standy state */
...
#if defined(CONFIG_TARGET_HI3531DV200) || defined(CONFIG_TARGET_HI3535AV100) || \
	defined(CONFIG_TARGET_HI3521DV200) || defined(CONFIG_TARGET_HI3520DV500)
#define EMMC_BUS_WIDTH_8BIT	8
#define EMMC_BUS_WIDTH_4BIT	4
	if (mmc->ocr_from_bootrom && !mmc->has_init) {
		/* Active state is Ds 25m 4/8bit */
		mmc->init_in_progress = 1;
		mmc->op_cond_pending = 0;
		mmc->rca = 1;
		...
		mmc->version = MMC_VERSION_UNKNOWN;
		mmc->high_capacity = ((mmc->ocr & OCR_HCS) == OCR_HCS);
		mmc->bus_width = readl(REG_BASE_SCTL + REG_SYSSTAT) &
				EMMC_BOOT_8BIT ? EMMC_BUS_WIDTH_8BIT :
						 EMMC_BUS_WIDTH_4BIT;
	}
#endif
