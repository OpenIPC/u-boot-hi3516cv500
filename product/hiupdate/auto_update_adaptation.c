				...
				unsigned long filesize, unsigned long pos)
{
	...
	if (off > SECTION_SIZE) {
		...
		debug("big chunk: layout.count:%u,layout.remain:%u,\
							...
							u16 type)
{
	...
	if (boot_medium_type == BOOT_FROM_SPI ||
		...
		debug("erase start:%lx erase end:%lx erase len:%lx\n",
...
write_op:
	...
	if (strstr(aufile[idx], "yaffs")) {
		...
		rc = write_ubi_fs(idx, offset, pbuf);
		if (rc) {
			printf("write ubi fs failed\n");
			return rc;
		}
	} else {
		rc = write_other_files(idx, offset, pbuf);
		if (rc) {
...
#else
void get_sparse_header(int file_idx, struct flash_update_parm *update_parm)
{
#if (defined CONFIG_EMMC) || (defined CONFIG_CMD_UFS)
	int ret;
...
void get_section_size(int file_idx, struct flash_update_parm *update_parm)
{
#ifdef CONFIG_CMD_NAND
	if (strstr(aufile[file_idx], "yaffs2")) {
		if (update_parm->sz > SECTION_SIZE)
			update_parm->section_size = (SECTION_SIZE / (nand_flash->writesize + nand_flash->oobsize)) *
...
void segment_data_save(int file_idx, struct flash_update_parm *update_parm)
{
	unsigned int opsz;
	int ret;
	...
	for (int segment = 0; segment < update_parm->nsect + update_parm->extra; segment++) {
		...
		ret = file_fat_read_at(aufile[file_idx], update_parm->pos, LOAD_ADDR, opsz, &update_parm->actread);
		if (ret)
		...
		if (strstr(aufile[file_idx], "ext4") || strstr(aufile[file_idx], "ufs")) {
			...
			debug("nsect:%d,remain:%u,pos:%lld\n", update_parm->nsect, update_parm->remain, update_parm->pos);
		}
...
{
	int i;
	struct flash_update_parm update_parm = {0};
